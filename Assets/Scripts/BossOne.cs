...
public class BossOne : MonoBehaviour
{
	...
	void Update()
	{
		...
		if ((Time.time - startTime) > 70f)
		{
			...
			{
				if (enemy.transform.position.x > -3.75f)
				{
					input.x = -1;
				}
				else if(enemy.transform.position.x < -4.25f)
				{
					input.x = 1;
				}
				else
			...
			{
				if (enemy.transform.position.x > 4.25f)
				{
					input.x = -1;
				}
				else if(enemy.transform.position.x < 3.75f)
				{
					input.x = 1;
				}
				else
		...
		// If the boss touches the ground, they will jump.
		if(Mathf.Abs(enemy.transform.position.x - player.transform.position.x) < 0.1f)
		{
			input.y = 0;
			jump = false;
		}
		else if(groundState.isGround() && (Time.time - startTime) < 71f)
		{
			input.y = 1;
			jump = true;
		}
	
		// If the boss is touching the wall, they will bounce off of it.
		...
		// If the player is below the boss, an the boss is touching the wall, the boss will fall down.
